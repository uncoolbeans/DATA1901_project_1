...
A limitation faced with this data set is the fact that the symptoms and their severity, for both `BSSQ` and `ASSQ` are self-reported, meaning that the interpretation of the scale can vary from person to person, making the reported values subjective and less reliable.

Another limitation is that VR experience is simply classified by a 'yes' or 'no', which is not a descriptive measure of the amount, frequency or how recently they used VR, which could be confounding factors. However, it was mentioned that participants who had used VR more than 10 times were excluded from the study.
